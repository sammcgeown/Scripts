...
# Only install DEM pre-requisites if set to $true
# DEM requirements are .NET 4.5 and SecondaryLogonService
$DEMOnly = $false
...
New-Item -Path $InetPubRoot -type directory -Force -ErrorAction SilentlyContinue | Out-Null
New-Item -Path $InetPubLog -type directory -Force -ErrorAction SilentlyContinue	 | Out-Null
New-Item -Path $InetPubWWWRoot -type directory -Force -ErrorAction SilentlyContinue | Out-Null
...
# Disable strict name checking, and add the load balancer URL to BackConnectionHostNames
# Fixes known issues where IaaS web role installation fails with HTTP 401.1 auth error
if($loadbalancerUrl) {
